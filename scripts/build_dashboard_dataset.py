...
def _find_parquet(filename: str) -> Path | None:
    candidates = [
        DATA_DIR / filename,
...
def build_dataset():
    ...
    pasajes_path = _find_parquet("vuelos_pasajes.parquet")
    fids_path = _find_parquet("vuelos_fids_naabol.parquet")

    pasajes_records = []
    if pasajes_path and pasajes_path.exists():
        try:
    ...
    fids_records = []
    if fids_path and fids_path.exists():
        try:
    ...
    try:
        try:
            ...
            from scripts.fetch_pasajes import TARIFAS_REFERENCIA_VUELOS, BANDAS_TARIFARIAS_BUSES
